...
export class FileHandlerImpl<TState extends VersionedState, TActionType extends string, TAction extends Action | TypedAction<TActionType>, TLoggerConfig>
  implements FileHandler<TState, TActionType, TAction> {
  ...
  constructor(private logger: Logger<TLoggerConfig>) {
    ...
    this.file
      .pipe(
        ...
        exhaustMap(file => writeFileAtomic(createFilePath(this.config.fileName, this.config.path), JSON.stringify(file))),
      )
      .subscribe(
        () => {
  ...
  loadState(): Observable<TState | undefined> {
    this.fileLoadLatch = new AsyncSubject();
    const fileObservable = fileExists(createFilePath(this.config.fileName, this.config.path))
      .pipe(
        exhaustMap(exists => {
          if (exists) {
            if (this.config.migration) {
              return readFile(createFilePath(this.config.fileName, this.config.path)).pipe(
                ...
                exhaustMap(file => writeFileAtomic(createFilePath(this.config.fileName, this.config.path), JSON.stringify(file)).pipe(map(() => file))),
              );
            }
            return readFile(createFilePath(this.config.fileName, this.config.path)).pipe(map(currentFileString => JSON.parse(currentFileString) as TState));
          }
          return of(undefined);
        }),
      )
  ...
  saveState(state: TState | undefined) {
    this.fileLoadLatch.pipe(take(1)).subscribe(
      () => {
